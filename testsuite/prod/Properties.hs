...
prop_T27 x = rev x =:= qrev x Nil


prop_T28 x = revflat x =:= qrevflat x Nil


prop_T29 x = rev (qrev x Nil) =:= x


prop_T30 x = rev (rev x ++ Nil) =:= x


prop_T31 x = qrev (qrev x Nil) Nil =:= x
...
prop_T39 x y z = givenBool (x `elem` ndrop y z) (proveBool (x `elem` z))
...
prop_T48 x = nlength (isort x) =:= nlength x
