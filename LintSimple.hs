...
data Err v
    ...
    | CaseWithoutAlts (Body (Typed v))
    | AltsRHSIllTyped (Body (Typed v)) [Type v]
...
ppErr p pt err = case err of
    ...
    CaseWithoutAlts e -> "Case without alternatives: " <+> ppBody pt e
    AltsRHSIllTyped e ts -> sep $
        "Alternatives in case differ in type:":ppBody pt e:map (ppType 0 p) ts
...
lintFnsAnd fns m = insertVars (map fn_name fns)
                              (mapM_ (lintBody . fn_body) fns >> m)

lintBody :: Ord v => Body (Typed v) -> LintM v (Type v)
lintBody b = case b of
    Case e alts -> do
        ts <- lintExpr e
        tys <- mapM (lintAlt ts) alts
        case tys of
            [] -> report (CaseWithoutAlts b) >> return Star
            t:tys' -> do
                unless (all (eqType t) tys') (report (AltsRHSIllTyped b tys))
                return t
    Body e -> lintExpr e
...
lintAlt t0 (p,rhs) = lintPat t0 p >> lintBody rhs
